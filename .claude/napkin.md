...
4. **[2026-07-18] URP shaders built purely via `Shader.Find` at runtime render black on-device (works fine in Editor)**
   Do instead: if code creates Materials at runtime (`new Material(Shader.Find("Universal Render Pipeline/Lit"))`) with no Material *asset* in the project referencing that shader, Unity's Android build strips the shader's compiled variants (Editor has every shader loaded, so it never reproduces there) — app launches with no crash/exception in `adb logcat`, just a black screen. Fix: add the shader(s) to Project Settings → Graphics → **Always Included Shaders** (`ProjectSettings/GraphicsSettings.asset`, `m_AlwaysIncludedShaders` — editable directly as YAML, no Editor UI needed; each entry is `{fileID: 4800000, guid: <shader's .meta guid>, type: 3}`). Also check `ProjectSettings/EditorBuildSettings.asset`'s `m_Scenes` isn't empty — an empty list means the build silently falls back to whichever scene happened to be open in the Editor rather than a reliable explicit list.
